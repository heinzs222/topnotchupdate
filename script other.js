...
`;
  ...
  function showFabricItems(categoryKey, subCategoryKey, folderPath, fileNames) {
    ...
    // Add a Back button: go back to sub-categories if there was one, else top-level.
  ...
  function createSubCategoryCard(
    ...
    // Build the folder path based on the category and sub-category.
    // For example: "./assets/fabric/Colour/Beige/"
  ...
  function showSubCategories(categoryKey) {
    ...
    if (Array.isArray(categoryData)) {
      // For categories like "All Fabrics" that are an array,
      // assume the folder path is in "./assets/fabric/All Fabrics/"
      const folderPath = "./assets/fabric/All Fabrics/";
      showFabricItems(categoryKey, null, folderPath, categoryData);
    } else {
      // For categories with sub-categories (objects), list each sub-category.
      const cardsWrapper = document.createElement("div");
      cardsWrapper.className = "cards-wrapper";
  ...
  function createTopLevelCategoryCard(categoryKey, index) {
    ...
    // Create details area that displays the category name
    const cardDetails = document.createElement("div");
    cardDetails.className = "card_cardDetails";
    const cardText = document.createElement("p");
    cardText.className = "card_cardText";
    ...
    // When clicked, show the sub-categories (if any) or the fabric images
    cardContainer.addEventListener("click", () => {
      showSubCategories(categoryKey);
    });
  ...
  function initializeTextureButtons() {
    ...
    // Loop over the top-level categories: Colour, Design, Event, All Fabrics
    ...
    // (Optionally) initialize mobile slider if needed
    if (window.matchMedia("(max-width: 1024.9px)").matches) {
      initializeCardsSlider();
    }
  }
  ...
  function createFabricCard(categoryKey, item, index, folderPath) {
    ...
    // When the card is clicked, select the fabric.
