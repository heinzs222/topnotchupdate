...
`;
  ...
  function applyTexture(url) {
    ...
    // If the requested texture is already active, do nothing.
    if (material.diffuseTexture && material.diffuseTexture.name === url) {
      return;
    }

    // Set a small placeholder texture immediately.
    // Ensure you have a very small and optimized placeholder image.
    const placeholderUrl = "./assets/fabric/placeholder.webp"; // <-- create/optimize this image
    let placeholder = new BABYLON.Texture(
      placeholderUrl,
      scene,
      false,
      true,
      BABYLON.Texture.BILINEAR_SAMPLINGMODE,
      () => {
        console.log(`Placeholder texture loaded: ${placeholderUrl}`);
      },
      (msg, exception) => {
        console.error(
          `Failed to load placeholder texture: ${placeholderUrl}`,
          msg,
          exception
        );
      }
    );
    placeholder.uScale = 5.0;
    placeholder.vScale = 5.0;
    material.diffuseTexture = placeholder;
    material.diffuseTexture.name = placeholderUrl;

    // Now create the high-res texture.
    let highResTexture = new BABYLON.Texture(
      url,
      scene,
      false, // noMipmap - set to false to generate mipmaps if needed
      true, // invertY (if needed)
      BABYLON.Texture.TRILINEAR_SAMPLINGMODE,
      () => {
        console.log(`High-res texture loaded: ${url}`);
        // Swap the placeholder with the high-res texture.
        material.diffuseTexture = highResTexture;
        material.diffuseTexture.name = url;
      },
      (msg, exception) => {
        console.error(`Failed to load texture: ${url}`, msg, exception);
      }
    );
    highResTexture.uScale = 5.0;
    highResTexture.vScale = 5.0;
